<template>
    <div class="flex items-center border-t py-6 border-gray-700 text-white">
        ...
        <p class="w-[20%] pl-[15px] text-[15px]">{{infoPurchase.transaction_name}}</p>
        <p class="w-[20%] text-[15px]">{{infoPurchase.transaction_amount}}€</p>
        <p class="w-[20%] text-[15px]">{{infoPurchase.formatted_date}}</p>
        <p class="w-[15%] text-[15px]">{{infoPurchase?.count_transaction}}x</p>
...
<script setup>
    ...
    import EditDelete from '@/components/overlay/EditDelete.vue';
