    ...
    <div class="font-main-font flex flex-col bg-main-bg w-[100%]">
        ...
        <main class="ml-[calc(20px+70px+20px)] mr-custom-margin-main flex flex-col mt-[20px]">
            ...
            <section class="flex gap-[20px] justify-around ">
                ...
                <ContainerStatMonth :svg="svgConfig('restaurant', 'bg-gradient-vanusa')" :colorValue="'text-white'"  :strValue="'Loyer'" :nameEconomy="'Plus gros prélèvement / Catégorie'" :width="'w-[25%]'" />
            </section>

            <section class="flex justify-between ">
                <ContainerListPurchases class="w-[calc(50%-10px)]" :title="'Historique des transactions'" :purchaseType="'standard'"  :svg="svgConfig('restaurant', 'bg-gradient-blue', '50px')" />
                <ContainerListPurchases class=" w-[calc(50%-10px)]" :title="'Historique des prélèvements'" :purchaseType="'recuring'" :svg="svgConfig('balance', 'bg-gradient-vanusa', '50px')" />
            </section>
